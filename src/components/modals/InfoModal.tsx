...
export const InfoModal = ({ isOpen, handleClose }: Props) => {
  ...
  return (
    <BaseModal title={t('howToPlay')} isOpen={isOpen} handleClose={handleClose}>
      ...
      <p className="text-sm text-gray-50">{t('correctSpotInstructions')}</p>
      <div className="flex justify-center mb-1 mt-4">
      ...
      <p className="text-sm text-gray-50">{t('wrongSpotInstructions')}</p>
      ...
      <p className="text-sm text-gray-50">{t('notInWordInstructions')}</p>
    </BaseModal>
  )
}
